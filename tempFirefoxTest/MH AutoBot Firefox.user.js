    ...
    if (isKingReward) {
        ...
        if (hornTime <= 0) {
            ...
            if (enableTrapCheck) {
                // update timer
                if (!aggressiveMode) {
                    displayTimer('Horn: ' + timeformat(hornTime) + ' | Check: ' + timeformat(checkTime), timeformat(hornTime) + '  <i>(included extra ' + timeformat(hornTimeDelay) + ' delay & +/- 5 seconds different from MouseHunt timer)</i>', timeformat(checkTime) + '  <i>(included extra ' + timeformat(checkTimeDelay) + ' delay)</i>');
                } else {
                    displayTimer('Horn: ' + timeformat(hornTime) + ' | Check: ' + timeformat(checkTime), timeformat(hornTime) + '  <i>(lot faster than MouseHunt timer)</i>', timeformat(checkTime) + '  <i>(included extra ' + timeformat(checkTimeDelay) + ' delay)</i>');
                }
            } else {
                // update timer
                if (!aggressiveMode) {
                    displayTimer('Horn: ' + timeformat(hornTime), timeformat(hornTime) + '  <i>(included extra ' + timeformat(hornTimeDelay) + ' delay & +/- 5 seconds different from MouseHunt timer)</i>', '-');
                    ...
                    displayTimer('Horn: ' + timeformat(hornTime), timeformat(hornTime) + '  <i>(lot faster than MouseHunt timer)</i>', '-');
...
function embedTimer(targetPage) {
    try {
        ...
        if (debug) cosole.log('embedTimer error - ' + e)
    }
}
...
function addGoogleAd() {
        ...
        if (headerElement) {
            ...
            autoBotAdDivElement.innerHTML = '<script type="text/javascript"><!-- google_ad_client = "ca-pub-0646444153861496"; google_ad_slot = "5069542056"; google_ad_width = 728;google_ad_height = 90; //--> </script> <script type="text/javascript" src="http://pagead2.googlesyndication.com/pagead/show_ads.js"></script>';
...
function embedScript() {
        ...
        scriptNode.setAttribute('soundedHornAtt', 'false');
        scriptNode.innerHTML = 'function soundedHorn() {    var scriptNode = document.getElementById("scriptNode");    if (scriptNode) {    \tscriptNode.setAttribute("soundedHornAtt", "true");    }    scriptNode = null;    }';
...
function kingRewardCountdownTimer() {
    ...
    if (kingPauseTime <= 0) {
        ...
        if (reloadKingReward) {
            // update timer
            displayTimer('King\'s Reward - Reload in ' + timeformat(kingPauseTime), 'Reloading in ' + timeformat(kingPauseTime), 'Reloading in ' + timeformat(kingPauseTime));
        }
        // set king reward sum time
